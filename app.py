...
아래 두 상수에 개인 키를 입력합니다. 실제 키가 들어간 파일은 공개 GitHub에
올리지 마세요. 환경변수 또는 Streamlit secrets로도 같은 이름을 설정할 수 있습니다.
...
def clean_key(name, constant):
    value = constant.strip() or os.getenv(name, "").strip()
    if not value:
        try:
...
def check_key(service, key):
    if not key or re.search(r"여기에|YOUR_|PASTE_", key, re.I):
        raise APIError(service, "KEY", "app.py 상단에 API 키를 입력한 뒤 다시 실행하세요.")
...
def main():
    ...
    krx_key, dart_key = clean_key("KRX_API_KEY", KRX_API_KEY), clean_key("DART_API_KEY", DART_API_KEY)
    ...
    if demo:
        ...
        if not krx_key:
            st.info("app.py 상단 KRX_API_KEY와 DART_API_KEY에 키를 입력하세요. 입력 후 ‘연결 진단’에서 서비스별 결과를 확인할 수 있습니다.")
            st.code('KRX_API_KEY = "여기에_KRX_키"\nDART_API_KEY = "여기에_OpenDART_키"', language="python")
